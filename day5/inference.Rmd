...
prom_perm <- promotions %>%
  ...
  summarize(diff_perm = diff(prop_purchase_perm),
            diff_orig = diff(prop_purchase_orig)) 
...
#### Now calculate the observed difference in proportion
...
```{r}
print(mean_income_null %>% summarize(sd(stat)))
mean_income_null %>%
...
# Calculate p-value
point_income_null %>%
  filter(mean_obs_inc < stat) %>%
  summarize(n() / 1000)
```

#### Confidence interval of mean income

```{r}
t.test(income$Income, conf.level = 0.95)
```

#### Confidence interval of mean difference in paired data

```{r}
paired = read.csv("E:/Training_S/Day 5/paired.csv")
str(paired)
```

Calculate difference between val1 and val2

```{r}
paired = paired %>%
  mutate(difference = val2 - val1)
glimpse(paired)
```

#### Confidencce interval for mean difference

```{r}
t.test(paired$difference, conf.level = 0.95)
```

#### Testing mean

H0: There is no difference between average value of val1 and val2
H1: There is a difference between average value of val1 and val2

```{r}
t.test(paired$difference, null = 0, alternative = "two.sided")
```

Now, let's check how it differs from bootstrap calculation
...
#### P-value for mean difference using bootstrap

```{r}
paired_boot_test = paired %>%
  specify(response = difference) %>%
...
```{r}
paired_boot_test %>%
  filter(stat > mean_observed_diff) %>%
  summarize(2 * n() / 10000)
```

#### Comparing two means from two independent distributions

Suppose, Shwapno wants to increase its online shopping revenue. Executives from Shwapno has come up with two different schemes A nd B for this. When users of Shwapno access its website, they are randomly assigned to one of these schemes. That is, some users access Shwapno website refurbished with scheme 1 and some users access Shwapno website with scheme 2.There is also information system which records the time an user expends in browsing the website. Further assume that for this study, we consider only those users who have already used Shwapno online or at least browsed Shwapno and there are records of their activity time in Shwapno's data storage. Now, Shwapno wants to know whether scheme B helps more to increse user's online activity at Shwapno's website than the scheme A does.

Let's load the data first.

```{r}
scheme = read.csv("E:/Training_S/Day 5/scheme.csv")
head(scheme)
```

First calculate the difference in user experience time
...
```{r}
(observed_diff = scheme %>%
  group_by(scheme) %>%
  summarize(mean_diff = mean(change)) %>%
  pull() %>%
  diff())
```

Using randomization to generate the null distribution of difference in means

```{r}
mean_bootstrap_difference = scheme %>%
  specify(change ~ scheme) %>%
  hypothesize(null = "independence") %>%
...
We can conclude that scheme B is more effective than scheme A.

#### t-test for comparing means

```{r}
t.test(change ~ scheme, data = scheme)
```


### ANOVA -- Comparing many means

H0: The mean of a numeric variable is the same across all classes under consideration
HA: The mean of this numeric variable is different between at least one of the classes

![](C:/Users/ERI/inference_3.png)

#### Conditions for ANOVA

* **Independence:** observations have to be independent, classes have to be independent also
* **Approximate normality:** response variable needs to be nearly normal within each class
* **Equal variance:** Groups need to have more or less equal variance, homoscedasticity

```{r}
results = data.frame(group=c(rep("A", 100), rep("B", 120), rep("C", 150), rep("D", 130)),
                     value = c(rnorm(100, 200, 20), rnorm(120, 189, 22), rnorm(150, 230, 24), rnorm(130, 196, 22)))
```

```{r}
library(broom)
(model = aov(value ~ group, results) %>%
  tidy())
```

This says that there are differences in means but not among which. We can determine the groups that have differences using **pairwise.t.test**

```{r}
pairwise.t.test(results$group, results$value, p.adjust = "none") %>%
  tidy()
```

TukeyHSD test controls for type 1 error.

```{r}
TukeyHSD(aov(value ~ group, results))
```
